...
const DB_FILE = path.join(DATA_DIR, "storage-io-history.db");

/**
 * ディスクI/Oのsnapshot(storageモジュールの`/io`が返す値)を
 * SQLiteに記録し、過去の推移を閲覧できるようにする。
 *
 * MonitoringStoreと同じ方針で、スキーマは正規化せず`collectIoSnapshot()`が返す
 * オブジェクトをそのままJSON文字列として1サンプル=1行で保存する。
 */
export class StorageIoStore {
	...
	insert(snapshot: { timestamp: string }): void {
		try {
			...
			// 書き込み失敗(ディスク容量等)は記録処理自体を止めないよう握りつぶす
		}
	}
